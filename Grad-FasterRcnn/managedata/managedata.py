...
def gen_data_txt():
    label_sheet = xlrd.open_workbook('biaojishuju.xlsx').sheets()[0]
    graddata = open('grad_data.txt', 'w')
    stagdata = open('stag_data.txt', 'w')
    # for i in range(1, label_sheet.nrows):
    for i in range(1, 399):
        # for i in range(1, 1000):
        ImageName, Grading, Staging, x1, y1, x2, y2 = label_sheet.row_values(i)
        graddata.write('./TrainingData/'+str(ImageName).strip('\'')+'.jpg,'
                       + str(int(x1))+','+str(int(y1))+','+str(int(x2))+','+str(int(y2))+','
                       + str(Grading).strip('\'')+'\n')
        stagdata.write('./TrainingData/' + str(ImageName).strip('\'') + '.jpg,'
                       + str(int(x1))+','+str(int(y1))+','+str(int(x2))+','+str(int(y2))+','
                       + str(Staging).strip('\'')+'\n')
    graddata.close()
    stagdata.close()
...
def gen_data_txt_with_person():
    label_sheet = xlrd.open_workbook('biaojishujuandperson.xlsx').sheets()[0]
    graddata = open('grad_data_with_person.txt', 'w')
    stagdata = open('stag_data_with_person.txt', 'w')
    personold = '0'
    for i in range(1, label_sheet.nrows):
    # for i in range(1, 399):
        # for i in range(1, 1000):
        ImageName, Grading, Staging, x1, y1, x2, y2, person = label_sheet.row_values(i)
        person = str(int(person))
        if person != personold:
            personold = person
            continue
        if person == '2' or person=='9' or person=='21' or person=='13':
            continue
        graddata.write('./TrainingData/' + str(ImageName).strip('\'') + '.jpg,'
                       + str(int(x1)) + ',' + str(int(y1)) + ',' + str(int(x2)) + ',' + str(int(y2)) + ','
                       + str(Grading).strip('\'') + '\n')
